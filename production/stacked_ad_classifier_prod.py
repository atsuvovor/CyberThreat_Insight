...
def predict_new_data(
    ...
    train_X_scaled = np.load(f"{model_dir}/train_X_scaled.npy").astype(np.float32)

    # ---------------- LOAD DATA ----------------
    df_new = load_new_data(NEW_DATA_URL, label_col, ops_df)

    augmented_df = load_aumented_dataset(
        AUGMENTED_DATA_URL=None,
        LABEL_COL=label_col,
    ...
    print("[INFO] Inference input dtypes (before cast):")
    ...
    X_seq = X_new_scaled.reshape((X_new_scaled.shape[0], 1, X_new_scaled.shape[1]))
    recon_lstm = lstm_autoencoder.predict(X_seq, verbose=0)
    features_new["lstm_mse"] = np.mean((X_seq - recon_lstm) ** 2, axis=(1, 2))

    # ---------------- STACK FEATURES ----------------
    X_new_ext = np.hstack([X_new_scaled, features_new.values])
    rf_proba = rf_base.predict_proba(X_new_ext)
    X_new_stack = np.hstack([X_new_ext, rf_proba])

    # ---------------- FINAL PREDICTION ----------------
    y_pred = gb_meta.predict(X_new_stack)

    df_new["Predicted Threat Level"] = y_pred
    df_new["anomaly_score"] = features_new.mean(axis=1)
    df_new["predicted_anomaly"] = (y_pred >= 2).astype(int)
